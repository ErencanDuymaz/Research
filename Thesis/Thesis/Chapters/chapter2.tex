...
Relation between grid frequency and the synchronous speed is given in Eq. (\ref{syncspeed}) in terms of rpm where $n_{s}$ is the synchronous speed in rpm, $f$ is the grid frequency in Hz, $p$ is the number of poles of the generator \cite{Kundur}. Since the grid frequency is proportional to synchronous speed, the generator speed of synchronous machines changes according to grid frequency. 
\begin{equation}
n_{s}=\frac{120f}{p}
\label{syncspeed}
\end{equation}
...
\begin{figure}[h!]
	...
	\caption[Frequency Behaviour in Electric Grid with the Water Level in a Container Analogy]{Frequency Behaviour in Electric Grid with the Water Level in a Container Analogy \cite{Eto2010}}
...
\begin{figure}[h!]
	...
	\caption[Variation of the Frequency in a Typical Day (06 Dec 2018)]{Variation of the Frequency in a Typical Day (06 Dec 2018) \cite{teiasfreq}}
	\label{06decfreq}
\end{figure}
...
\begin{figure}[h!]
	...
	\caption[Grid Frequency Control in England and Wales]{Grid Frequency Control in England and Wales \cite{Ekanayake2008}, \cite{Erinmez1999}}
	\label{freqcontrol}
\end{figure}
...
\end{table}
\newpage
In addition to FIT, energy provider can benefit from additional incentives as long as some parts of the system is produced inside Turkey. For instance, by choosing a tower of a wind turbine which is a domestic production, an additional price is given to energy provider as local-bonus content. The local-bonus contents for wind turbines are listed in Table \ref{price2}. The renewable energy systems can benefit from local bonuc content during 5 years period. It should be noted that the plants that will be commisioned after 2020 will not benefit from FIT and local content bonus but will take place in day ahead and intra-day markets.\par
\begin{table}[h!]
	\centering
	\begin{tabular}{cc}
		\hline
